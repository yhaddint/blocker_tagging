...
%     CAL_u = ((randi(2,P,blk_num)*2-3)+1j*(randi(2,P,blk_num)*2-3))/sqrt(2);
%     CAL_u = exp(1j*2*pi*rand(P,blk_num));
%     CAL_u(:,[1,2,3,5,6]) = 0;
%     CAL_l = ((randi(2,P,blk_num)*2-3)+1j*(randi(2,P,blk_num)*2-3))/sqrt(2);
    ...
    for kk=1:blk_num
        for ll=1:blk_num
            if kk==ll
                sig_mix0_u = sig_mix0_u+sqrt(4*sig_pow0(kk))*real(sig_cache(:,kk)*PhaseShift(kk).*conj(CAL_u(:,kk)));
                sig_mix1_u = sig_mix1_u+sqrt(4*sig_pow1(kk))*real(sig_cache(:,kk)*PhaseShift(kk).*conj(CAL_u(:,kk)));
%                 sig_mix0_l = sig_mix0_l+sqrt(2*sig_pow0(kk))*real(sig_cache(:,kk)*PhaseShift(kk).*conj(CAL_l(:,kk)));
%                 sig_mix1_l = sig_mix1_l+sqrt(2*sig_pow1(kk))*real(sig_cache(:,kk)*PhaseShift(kk).*conj(CAL_l(:,kk)));
            else
                carrier = exp(1j*((kk-ll)/SampleNumberAve*2*pi*(1:P)'))*PhaseShift(kk);
                % H0 part
                sig_mix0_u = sig_mix0_u + sqrt(4*sig_pow0(kk))...
                    *real(sig_cache(:,kk).*carrier.*conj(CAL_u(:,ll)));
%                 sig_mix0_l = sig_mix0_l + sqrt(2*sig_pow0(kk))...
%                     *real(sig_cache(:,kk).*carrier.*conj(CAL_l(:,ll)));
                % H1 part
                sig_mix1_u = sig_mix1_u + sqrt(4*sig_pow1(kk))...
                    *real(sig_cache(:,kk).*carrier.*conj(CAL_u(:,ll)));
%                 sig_mix1_l = sig_mix1_l + sqrt(2*sig_pow1(kk))...
%                     *real(sig_cache(:,kk).*carrier.*conj(CAL_u(:,ll)));
            end
        end
    end
...
%     freqz(bhi,1)
%     yout_u = filter(bhi,1,sig_mix1_u.*real(CAL_u(:,1)));
    yout_u = filter(bhi,1,sig_mix1_u.*CAL_u(:,1));
%     yout_u_conj = filter(bhi,1,sig_mix1_u.*conj(CAL_u(:,1)));
%     yout_u = sig_mix1_u.*CAL_u(:,1);

%     yout_l = filter(bhi,1,sig_mix1_l.*real(CAL_l(:,1)));
...
%     yout_u = filter(bhi,1,sig_mix0_u.*real(CAL_u(:,1)));
    yout_u = filter(bhi,1,sig_mix0_u.*CAL_u(:,1));
%     yout_u_conj = filter(bhi,1,sig_mix0_u.*conj(CAL_u(:,1)));


%     yout_u = sig_mix0_u.*CAL_u(:,1);
%     yout_l = filter(bhi,1,sig_mix0_l.*real(CAL_l(:,1)));
...
%     Sam_end = 100+Sam_Num;
%     bhi = fir1(200,0.9/SampleNumberAve,'low');
% 
%     
%     for ll=1:6
%         y4_H1_orig(:,ll) = sig_mix0_u(1:Sam_end).*CAL_u(1:Sam_end,ll).*exp(-1j*(4-ll)/32*2*pi*(1:Sam_end)');
%         y4_H1_conj(:,ll) = sig_mix0_u(1:Sam_end).*conj(CAL_u(1:Sam_end,ll)).*exp(1j*(4-ll)/32*2*pi*(1:Sam_end)');
% 
%         temp_H1_orig_sep(:,ll) = filter(bhi,1,y4_H1_orig(:,ll));
%         temp_H1_conj_sep(:,ll) = filter(bhi,1,y4_H1_conj(:,ll));
%     end
%     
%     temp_H1_orig = sum(temp_H1_orig_sep,2)/6;
%     temp_H1_conj = sum(temp_H1_conj_sep,2)/6;
%     
%     y_j4_hat_H1 = zeros(Sam_Num,1);
%     for jj=1:6
%         if jj==4
%             y_j4_hat_H1 = y_j4_hat_H1...
%                 +temp_H1_orig(101:Sam_end).*conj(CAL_u(1:Sam_Num,4))...
%                 +temp_H1_conj(101:Sam_end).*CAL_u(1:Sam_Num,4);
%         else
%         y_j4_hat_H1 = y_j4_hat_H1...
%             +temp_H1_orig(101:Sam_end).*exp(1j*(4-jj)/32*2*pi*(1:Sam_Num)').*conj(CAL_u(1:Sam_Num,jj))...
%             +temp_H1_conj(101:Sam_end).*exp(-1j*(4-jj)/32*2*pi*(1:Sam_Num)').*CAL_u(1:Sam_Num,jj);
%         end
%     end
%     post_comp_H1 = (sig_mix1_u(1:Sam_Num)-y_j4_hat_H1);
%     result_H1_orig = filter(bhi,1,post_comp_H1.*CAL_u(1:Sam_Num,1));
%     result_H1_conj = filter(bhi,1,post_comp_H1.*conj(CAL_u(1:Sam_Num,1)));
%     results_SA_IC_H1(pow_index,runindex) = (result_H1_orig.'*result_H1_conj)/Sam_Num;
% 
%     for ll=1:6
%     y4_H0_orig(:,ll) = sig_mix0_u(1:Sam_end).*CAL_u(1:Sam_end,ll).*exp(-1j*(4-ll)/32*2*pi*(1:Sam_end)');
%     y4_H0_conj(:,ll) = sig_mix0_u(1:Sam_end).*conj(CAL_u(1:Sam_end,ll)).*exp(1j*(4-ll)/32*2*pi*(1:Sam_end)');
%     
%     temp_H0_orig_sep(:,ll) = filter(bhi,1,y4_H0_orig(:,ll));
%     temp_H0_conj_sep(:,ll) = filter(bhi,1,y4_H0_conj(:,ll));
%     end
%     
%     temp_H0_orig = sum(temp_H0_orig_sep,2)/6;
%     temp_H0_conj = sum(temp_H0_conj_sep,2)/6;
%     
%     y_j4_hat_H0 = zeros(Sam_Num,1);
%     for jj=1:6
%         if jj==4
%             y_j4_hat_H0 = y_j4_hat_H0...
%                 +temp_H0_orig(101:Sam_end).*conj(CAL_u(1:Sam_Num,4))...
%                 +temp_H0_conj(101:Sam_end).*CAL_u(1:Sam_Num,4);
%         else
%         y_j4_hat_H0 = y_j4_hat_H0...
%             +temp_H0_orig(101:Sam_end).*exp(1j*(4-jj)/32*2*pi*(1:Sam_Num)').*conj(CAL_u(1:Sam_Num,jj))...
%             +temp_H0_conj(101:Sam_end).*exp(-1j*(4-jj)/32*2*pi*(1:Sam_Num)').*CAL_u(1:Sam_Num,jj);
%         end
%     end
%     post_comp_H0 = (sig_mix0_u(1:Sam_Num)-y_j4_hat_H0);
%     result_H0_orig = filter(bhi,1,post_comp_H0.*CAL_u(1:Sam_Num,1));
%     result_H0_conj = filter(bhi,1,post_comp_H0.*conj(CAL_u(1:Sam_Num,1)));
%     results_SA_IC_H0(pow_index,runindex) = (result_H0_orig.'*result_H0_conj)/Sam_Num;
%     
    %%
%     NNN = 1e2:50e2;
%     temp_H0 = (sum(temp_H0_orig(101:Sam_end,1:6),2)+sum(temp_H0_conj(101:Sam_end,1:6),2))/6;
% %       temp_H0 = temp_H0_orig_1(101:Sam_end)+temp_H0_conj_1(101:Sam_end);
%     figure 
%     plot(temp_H0(NNN))
%     hold on
%     plot(sqrt(2)*real(sig_cache(NNN,4)*PhaseShift(4)));hold on
%     plot(temp_H0(NNN)-sqrt(2)*real(sig_cache(NNN,4)*PhaseShift(4)))
%     % plot(y4_H0(NNN))
%     legend('hat','true','diff')
%     MSE = 10*log10(norm(sqrt(2)*real(sig_cache(NNN,4)*PhaseShift(4)))^2/...
%     norm(temp_H0(NNN)-sqrt(2)*real(sig_cache(NNN,4)*PhaseShift(4)))^2);
%     display([num2str(MSE),'dB'])
end

%

temp = sort(real(results_SA_H0(pow_index,:)),'ascend');
TH_SA(pow_index) = temp(runtimes*0.9);
pd_SA(pow_index) = sum(real(results_SA_H1(pow_index,:))>TH_SA(pow_index))/runtimes;
%
...
% temp = sort(real(results_SA_IC_H0(pow_index,:)),'ascend');
% TH_SA_IC(pow_index) = temp(runtimes*0.9);
% pd_SA_IC(pow_index) = sum(real(results_SA_IC_H1(pow_index,:))>TH_SA_IC(pow_index))/runtimes;
...
% test11 = temp_H0_orig(100+NNN).*conj(CAL_u(NNN,4))...
%        + temp_H0_conj(100+NNN).*CAL_u(NNN,4);
% test12 = temp_H0_orig(100+NNN).*exp(1j*(4-1)/32*2*pi*(NNN)').*conj(CAL_u(NNN,4))...
%     +    temp_H0_conj(100+NNN).*exp(-1j*(4-1)/32*2*pi*(NNN)').*CAL_u(NNN,4);
% 
% test1 = test11+test12;
% 
% 
% test21 = real(sqrt(2)*sig_cache(NNN,4)*PhaseShift(4).*conj(CAL_u(NNN,4)));
% test22 = real(sqrt(2)*sig_cache(NNN,4)*PhaseShift(4)...
%     .*exp(1j*(4-1)/32*2*pi*(NNN)').*conj(CAL_u(NNN,4)));
% 
% test2 = test21+test22;
% 
% figure
...
% grid on
% legend('single-arm (ED0)','double-arm, proposed (ED1) ','double-arm, optimal (ED2)','single-arm, active IC')

% legend('single-arm (ED0)','single-arm, active IC')

%% test distribution of ED0
pow_index = 2;
s_pow = 10^(pow_pool(pow_index)/10)*(1+3/32+2/32*5);
n_pow = 0.7/32+2/32*(6-1);
% n_pow = 0;
N_prime = Sam_Num/32;
...
% H1 theoretical
mu = s_pow+n_pow;
sigma = sqrt((s_pow+n_pow)^2/(N_prime));
pdfxdata = linspace(mu-4*sigma,mu+4*sigma,100);
figure(99)
plot(pdfxdata, normpdf(pdfxdata,mu,sigma),'linewidth',2);hold on
grid on
...
N_prime_ED0 = Sam_Num/(SampleNumberAve_ED0);
N_prime_ED1 = Sam_Num/SampleNumberAve_ED1;
N_prime_ED2 = Sam_Num/(SampleNumberAve_ED0);

blk_num = 6;
pow_pool_fine = -10-linspace(0,20,100);
% pow_pool_fine = -18;
for pow_index = 1: 100
    s_pow_ED0(pow_index) = 10^(pow_pool_fine(pow_index)/10)...
        *(1+3/SampleNumberAve_ED0+2/SampleNumberAve_ED0*(blk_num-1));
...
%     s_pow_ED2(pow_index) = 10^(pow_pool_fine(pow_index)/10)*(1+0.5/SampleNumberAve_ED2*(blk_num-1));
    ...
    n_pow_ED0 = 1/SampleNumberAve_ED0...
        +2/SampleNumberAve_ED0*(blk_num-1);
