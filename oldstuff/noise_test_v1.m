...
%  STSA is used and compared with other version
        ...
        for kk=1:blk_num
            ...
            for ii=1:blk_num
                if sig_table(ii,1)==kk
                    sig_sum0 = sig_sum0+sig(start_point(ii):start_point(ii)+P-1,ii);
                end
            end
            noise_normal = randn(P,1);
            for qq=1:7
                sig_sum = sig_sum0+sqrt(quan_noise(qq))*noise_normal;
    ...
    for qq=1:7
        for F_index = 1:F_num
            ...
            for ii = 1:runtimes/ave_num
                for kk=1:band_num
                    A(:,kk) = sum(squeeze(bins_bands((ii-1)*ave_num+1:ii*ave_num,:,kk,qq)))/ave_num;
                end
