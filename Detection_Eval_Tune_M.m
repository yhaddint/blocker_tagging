...
%  The script used for figure in ARR 2015 poster. Figure is saved in
%  figures folder
%  Running requires 15+ mins
...
for mm=1:mm_num
    ...
    pd_withFixed_pfa(mm,pindex)=Pd_MC(index,mm);
end
...
xmm=mm_range(1:8:end);
figure
plot_setting()
for pindex=1:length(powerdiff_range)
plot(xmm,pd_withFixed_pfa(1:8:end,pindex),color(pindex,:));hold on
end
grid on
legend('12dB Power Range','15dB Power Range','18dB Power Range')
