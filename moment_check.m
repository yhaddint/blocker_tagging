...
for ii=1:BLKcan_num
    ...
    sig_bb(:,ii) = temp_data(end-45*SampleperSymbol+1:end-5*SampleperSymbol+1);
...
for runindex=1:runtimes
    ...
    BLK(:,1)=BLK_r(Starindex:Starindex+N-1,tempindex);%+sig_bb_i(Starindex:Starindex+N-1,tempindex);
    BLK(:,2)=BLK_i(Starindex:Starindex+N-1,tempindex);
